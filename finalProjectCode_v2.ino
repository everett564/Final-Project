...
const float encTicksPerInch = -540;//-694.5; 
...
void setup() {
  ...
  flameController.defaultTolerance = 150;
...
void fanBeginSequence(){
  ...
  int diff = map(topIn-leftIn, minDiff, maxDiff, minHeight, maxHeight);
  esc.write(170);
  fanTimer = millis();
  ...
  lcd.clear();
  lcd.print(" X: ");
  lcd.print((int)(pos.x + frontUltra.getLastDist() * cos(pos.heading*M_PI/180)));
  lcd.print(" Y: ");
  lcd.print((int)(pos.y + frontUltra.getLastDist() * sin(pos.heading*M_PI/180)));
  lcd.setCursor(0,1);
  lcd.print(" Z: ");
  flame1 = (double) analogRead(LEFT_FLAME)*(1/800);
  flame2 = (double) analogRead(TOP_FLAME)*(1/800);
  height = (double)(8+sin((2*M_PI/180)*(1/(cos((2*M_PI/180)*((sq(flame1))+(sq(flame2))-42.25)/(2*flame1*flame2))))));
  lcd.print(height);
}
  ...
  switch (currentState)
    {
      ...
      case TurnRight:
        if(turningController.isInTolerance()){
          if(sideUltra.getLastDist()<wallDist){
            currentState = WallFollowing;
            rightTurnCount = 0;
          }
  ...
  switch (currentState)
    {
      ...
      case MoveForward:
        forwardInput = 0;
        forwardController.setGoal(17);//14);
      break;
      case MoveToWall:
        forwardInput = 0;
